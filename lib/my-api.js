...
export async function fetchThumbnail(title) {
  const res = await fetch(
    `https://en.wikipedia.org/w/api.php?action=query&format=json&prop=extracts%7Cpageimages%7Crevisions&titles=${title}%20|%20${title}_(country)&redirects=1&formatversion=2&exsentences=2&exintro=1&explaintext=1&piprop=thumbnail&pithumbsize=50&rvprop=timestamp`
  );
  const data = await res.json();
